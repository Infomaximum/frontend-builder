...
type ProxyConfig = {
   ...
   * Может быть указан вместе со схемой и портом, например "https://example.com:8091".
   * Если схема не указана, она определяется флагом -s/--https
   */
  host?: string;

  /**
   * Порт на который будет происходить проксирование всех запросов.
   * Игнорируется, если порт указан в host
   */
  port?: number;

  /**
   * Проверять ли SSL сертификат хоста, на который проксируются запросы.
   * По умолчанию true для https/wss. Для самоподписанных сертификатов нужно false.
   * Переопределяется флагами --secure/--no-secure
   */
  secure?: boolean;
};
